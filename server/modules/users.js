import { createUser, changeUser, deleteUser, createdUser, changedUser, deletedUser } from '../../protocol/protocol.js';
...
export default (server) => {
    server.channel('users', {
        ...
        filter(ctx, action) {
            return (otherCtx, otherAction) => {
                ...
                else if (deletedUser.match(otherAction) ||
    ...
    server.type(createUser, {
        ...
        async process(ctx, action) {
            ...
            await server.process(createdUser({ id: action.id, fields: action.fields }));
        }
    }); // queue = true
    ...
    server.type(changeUser, {
        ...
        async process(ctx, action) {
            ...
            await server.process(changedUser({ id: action.id, fields: action.fields }));
        }
    }); // queue = true
